...
_opts = [
    ...
    cfg.StrOpt('discovery_pxeip',
               default='192.0.2.1',
               help=('IP address of the TFTP server for discovery. Should '
                     'usually match the value of local_ip.')
               ),
...
def _check_hostname():
    ...
    detected_static_hostname = _run_command(args, name='hostnamectl')
    LOG.info('Static hostname detected as %s', detected_static_hostname)
    args = ['sudo', 'hostnamectl', '--transient']
    detected_transient_hostname = _run_command(args, name='hostnamectl')
    LOG.info('Transient hostname detected as %s', detected_transient_hostname)
    LOG.info('$HOSTNAME detected as %s', os.environ['HOSTNAME'])
    ...
    if detected_static_hostname != os.environ['HOSTNAME']:
        LOG.error('Static hostname "%s" does not match $HOSTNAME "%s".',
                  detected_static_hostname, os.environ['HOSTNAME'])
        LOG.error('Set $HOSTNAME to match or relogin so $HOSTNAME is '
                  'redefined.')
        raise RuntimeError('Static hostname does not match $HOSTNAME')

...
class TestGenerateEnvironment(base.BaseTestCase):
    ...
    def test_rhel7_distro(self, fake_distro):
        self.useFixture(fixtures.EnvironmentVariable('NODE_DIST', None))
        fake_distro.return_value = ['Red Hat Enterprise Linux Server 7.1']
        env = undercloud._generate_environment('.')
    ...
    @mock.patch('platform.linux_distribution')
    def test_centos7_distro(self, fake_distro):
        self.useFixture(fixtures.EnvironmentVariable('NODE_DIST', None))
        fake_distro.return_value = ['CentOS Linux release 7.1']
        env = undercloud._generate_environment('.')
    ...
    @mock.patch('platform.linux_distribution')
    def test_fedora_distro(self, fake_distro):
        self.useFixture(fixtures.EnvironmentVariable('NODE_DIST', None))
        fake_distro.return_value = ['Fedora Infinity + 1']
        self.assertRaises(RuntimeError, undercloud._generate_environment, '.')

    @mock.patch('platform.linux_distribution')
    def test_other_distro(self, fake_distro):
        self.useFixture(fixtures.EnvironmentVariable('NODE_DIST', None))
        fake_distro.return_value = ['Gentoo']
        self.assertRaises(RuntimeError, undercloud._generate_environment, '.')
...
class TestRunCommand(base.BaseTestCase):
    ...
    def test_run_live_command(self):
        output = undercloud._run_live_command(['echo', 'bar'])
        self.assertIn('bar\n', self.logger.output)

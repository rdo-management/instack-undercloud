...
A description of the Plan and Role concepts and the operations available.


Understanding Roles & Plans
---------------------------

Roles represent a baremetal node type which is defined by one or more TripleO
Heat Templates. Plans represent the full deployment and consist of one or more
roles and their related Parameters.

Roles
-----

The Roles included in the Tuskar API can be viewed like so::
...
Once you have the Plan UUID you can view more details about the about the
plan::
...
The output of both of these commands is the summary of a plan and it will
...
Note that in this example we are assigning the ``baremetal`` flavor to the
Compute role, but we need to include the role version when doing this. Making
the syntax ``-F [role-name]-[role-version]=[flavor-name]``.


Setting Scale values
~~~~~~~~~~~~~~~~~~~~

Similarly to assigning Flavors, scaling an individual role can be done like
this::

    $ openstack management plan set "[plan-uuid]" -S Compute-1=3

Here we are setting the Compute scale value to three, this means that it will
use three nodes when deployed.
...
To pass these directly to Heat, this can be done with::

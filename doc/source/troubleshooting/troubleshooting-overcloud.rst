...
This usually means some mismatch between resources that Nova expects to find
and resources that Ironic advertised to Nova. A few things should be checked
in this case:
...
#. Nova flavor that you are using strictly matches the Ironic node properties
...
#. Make sure that enough nodes are in ``available`` state according to
   ``ironic node-list``. Nodes in ``manageable`` state usually mean failed
   introspection.
...
#. Make sure nodes you're going to deploy to are not in maintenance mode.
   ...
   them and then remove maintenance mode::
    ...
    ironic node-update <IRONIC-NODE-UUID> replace maintenance=false

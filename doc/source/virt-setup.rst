...
This setup defines 5 virtual machines with 4GB of memory, 1 cpu and 40GB of
disk space on each [#]_.

The number of virtual machines, their ram and number of cores can be
customized by exporting ``NODE_COUNT``, ``NODE_MEM`` and ``NODE_CPU``. You
should only increase the memory or cpu count though or they will not match
the minimum required by the nova flavors.

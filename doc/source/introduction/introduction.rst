...
The basic deployment flow (as documented elsewhere in this guide) is illustrated below:

.. image:: ../_images/DeploymentFlow.svg

In summary the steps are:

1. Install and configure your undercloud node.
2. Register your baremetal nodes with Ironic (baremetal provisioning service).
3. Create (or download) images and upload them to Glance (image repository).
4. Optionally perform "advanced" introspection, benchmarks, hardware configuration and role-matching.
5. Plan your deployment (number of nodes per role, network configuration and other parameters)
6. Deploy the overcloud (driven via the Heat orchestration service).

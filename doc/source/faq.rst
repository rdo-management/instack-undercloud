...
bugs in our scripts), node can be in "AVAILABLE" state, which is meant for
deployment, not for discovery. You should advance node to "MANAGEABLE" state
...
to be in wrong state. Please refer to `upstream node states documentation

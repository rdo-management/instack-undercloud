...
To get the deployment plan UUID, run:
...
The deploy command will print the IP address of the overcloud controller node.
You will need to pass it to the next command, which performs post-deployment
configuration on the overcloud:

    source overcloudrc
    openstack overcloud postconfig "[overcloud_controler_ip]"

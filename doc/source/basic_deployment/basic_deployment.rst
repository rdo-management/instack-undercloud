...
To create an Overcloud public network, which is the network from which the
guests will get floating IPs, one can use::

    source ~/overcloudrc
    neutron net-create nova --router:external
    neutron subnet-create --name nova --enable_dhcp False \
      --allocation-pool start=172.16.23.140,end=172.16.23.240 \
      --gateway 172.16.23.251 nova 172.16.23.128/25

We named it "nova" because that's what tempest expects, based on nova.conf.
...
To use a VLAN, the following should work::
    ...
    neutron subnet-create --name nova --enable_dhcp False \
      --allocation-pool start=10.35.180.110,end=10.35.180.130 \
      --gateway 10.35.180.254 --cidr 10.35.180.0/24

Edit the network address ranges and VLAN id to match your needs, obviously.
